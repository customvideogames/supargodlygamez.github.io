...
(function ($, Drupal, drupalSettings) {
  ...
  */
    function loginHandler(event) {
      ...
      if($('#edit-name').length && $('form#user-login').length) {
        ...
        if(typeof email === 'undefined' || email === null || email === '') {
          ...
          error_msg = '<div class="messages--error">Please enter a valid email address.</div>';
          //console.log('Invalid email');
          isValid = false;
        } else {
      ...
      if(typeof pass === 'undefined' || pass === '' || pass.length < 6) {
      	...
      	error_msg += '<div class="messages--error">Your Password should be at least 6 characters long.</div>';
    ...
    function signUpUrlCallBack(){
      console.log("signUpUrlCallBack click handler");
      if(typeof signUpURL != "undefined" && signUpURL != null ) {
        window.location = window.location.protocol + window.location.hostname + signUpURL;
      } else {
        window.location = window.location.protocol + window.location.hostname + "/signup/b2df5a33";
      }
    }

    $('form#cmatgame-subscription-signup-form1 #edit-submit').click(loginHandler);
    if($('form#cmatgame-subscription-myaccount-login-form').length) {
      $('form#cmatgame-subscription-myaccount-login-form #edit-submit').click(loginHandler);
    } else if($('form#user-login').length) {
      $('form#user-login #edit-submit').click(loginHandler);
    }
    //attach
    jQuery('.signup-on-login-page').click(signUpUrlCallBack);
  //}
})(jQuery);
